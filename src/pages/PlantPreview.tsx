// src/pages/PlantPreviewView.tsx
import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { useAuth } from "../context/AuthContext";
import { getPlantByIdPublic } from "../api/plantService";
import type { TPlant } from "../types/plant";
import { FaArrowLeft, FaEdit } from "react-icons/fa";
import { motion } from "framer-motion";

export default function PlantPreviewView() {
  const { id } = useParams<{ id: string }>();
  const { user } = useAuth(); // don't use loading here
  ...
  if (fetching) return <p className="text-center mt-10">Loading plant...</p>;
